...
module BitbucketHelper
  ...
  def save_file
    File.open('/tmp/tasklistapp/TaskListApp/tasklist.txt', 'w') { |file| file.write("Changed text") }
  ...
  def create_file
    ...
    if !Dir['/tmp/tasklistapp/TaskListApp/tasklist.txt']
      File.open('/tmp/tasklistapp/TaskListApp/tasklist.txt', 'w') { |file| file.write('Task 1')}
    end
  end
end